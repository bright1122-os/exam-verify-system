...
export default function PrintQR() {
  ...
  if (loading) {
    return (
      <div className="min-h-[calc(100vh-64px)] flex items-center justify-center bg-parchment font-body text-charcoal">
        <div className="flex flex-col items-center gap-4">
          <div className="w-8 h-8 rounded-full border-2 border-clay border-t-transparent animate-spin" />
  ...
  return (
    <PageTransition>
      <div className="min-h-[calc(100vh-[6rem])] bg-parchment py-12 px-6 lg:px-8 font-body text-charcoal print:bg-white print:py-0 flex flex-col items-center">
        ...
        <div className="w-full max-w-lg relative perspective-1000 print:max-w-none print:shadow-none">
          ...
          <motion.div
            ...
            className="bg-white rounded-[2.5rem] p-10 lg:p-12 shadow-float border border-parchment-dark relative overflow-hidden print:border-none print:rounded-none"
          >
            {/* Elegant Background Texture / Shape */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-clay/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 pointer-events-none print:hidden" />

            {/* Header */}
            <div className="flex justify-between items-start mb-12 border-b border-parchment pb-8">
              <div>
                <div className="flex items-center gap-3 mb-2 opacity-60">
                  <Shield className="w-4 h-4 text-clay" />
                  <span className="text-sm font-semibold uppercase tracking-widest text-clay">Official Verification</span>
                </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row gap-8 sm:gap-12 items-center sm:items-start mb-12">
              ...
              <div className="flex-shrink-0 relative">
                <div className="relative z-10 p-2 bg-white rounded-3xl shadow-soft border border-parchment/50">
                  {studentData?.qr_generated ? (
                    <div className="overflow-hidden rounded-2xl">
                      <QRCodeSVG value={encryptedPayload || ''} size={160} level="H" includeMargin={true} />
                    </div>
                  ) : (
                    <div className="w-[160px] h-[160px] rounded-2xl border border-dashed border-charcoal/20 flex flex-col items-center justify-center text-charcoal-light bg-parchment/30">
              ...
              <div className="flex-1 flex flex-col items-center sm:items-start text-center sm:text-left">
                ...
                <div className="space-y-3 w-full max-w-[200px] mx-auto sm:mx-0">
                  <div className="flex justify-between items-center border-b border-parchment-dark pb-2">
            ...
            {/* Clearance Notice */}
            <div className="bg-parchment/50 rounded-2xl p-6 border border-parchment-dark relative">
              <div className="w-1.5 absolute left-0 top-6 bottom-6 bg-clay rounded-r-full" />
          ...
          >
            <div className="bg-white p-2 rounded-full shadow-float border border-parchment-dark flex gap-2">
              <button onClick={handleDownload} className="px-6 py-3 rounded-full flex items-center gap-2 hover:bg-parchment text-sm font-medium transition-colors">
                <Download className="w-4 h-4" /> Save Copy
              </button>
              <button onClick={handlePrint} className="bg-charcoal text-parchment px-6 py-3 rounded-full flex items-center gap-2 hover:bg-charcoal-light transition-colors text-sm font-medium">
