...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#f2f0e9] selection:bg-white/30 selection:text-white px-6 font-body py-12">
      <motion.div
      ...
      >
        <div className="container-editorial p-10 lg:p-12">
          ...
          <div className="text-center mb-10">
            <div className="flex justify-center mb-8">
              <div className="w-16 h-16 bg-white rounded-3xl flex items-center justify-center">
                <ShieldCheck className="w-8 h-8 text-charcoal" strokeWidth={1} />
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="pt-4">
              <button
                ...
                className={`w-full ${loading || authSuccess ? 'bg-white text-charcoal-light cursor-not-allowed shadow-none' : 'w-full bg-charcoal text-[#f2f0e9] hover:bg-charcoal/90'} py-4 rounded-full font-medium text-lg transition-all duration-500`}
              >
                <AnimatePresence mode="wait">
                  {authSuccess ? (
                    <motion.div key="success" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-center gap-2 text-[#f2f0e9]">
                      Authenticated
                    </motion.div>
                  ) : loading ? (
          ...
          <button
            ...
            type="button"
            className="w-full flex items-center justify-center gap-3 bg-white text-charcoal py-4 rounded-full font-medium hover:bg-white-dark/50 hover:shadow-soft transition-all duration-500 ease-organic"
          >
        ...
        <p className="text-center mt-10 text-charcoal/80 text-sm font-medium">
          First time here?{' '}
          <Link to="/auth/signup" className="text-white hover:text-charcoal transition-colors underline decoration-charcoal/40 underline-offset-4 pl-1">
            Create an account
          </Link>
        </p>

        <div className="text-center mt-8">
          <Link to="/" className="inline-block text-charcoal/50 hover:text-charcoal text-sm font-medium transition-colors">
            Return to homepage
          </Link>
        </div>
      </motion.div>
